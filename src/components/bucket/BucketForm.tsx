...
const BucketForm = ({ userId }: BucketFormProps) => {
  ...
  const { verify, isVerifying, isAddingCreds } = useBuckets({
    userId,
    onCreateComplete: (bucketName) => {
      setOpen(false);
      form.reset();
      toast({
        title: 'Bucket Added',
        description: `Your new bucket ${bucketName} has been successfully added.`,
        variant: 'success',
      });
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      verify(formattedValues);
    } catch (error) {
      // Error handling is done by the hooks
    }
  }

  const isFieldDisabled = isVerifying || isAddingCreds;
  return (
      ...
      <DialogTrigger asChild>
        <Button disabled={isFieldDisabled}>
          ...
          {isVerifying
            ? 'Verifying...'
            : isAddingCreds
            ? 'Verified, Now adding the bucket'
            : 'Add Bucket'}
        </Button>
      </DialogTrigger>
        ...
        <Form {...form}>
          ...
          >
            ...
            <Button type="submit" disabled={isFieldDisabled}>
              ...
              {isAddingCreds ? 'Adding the bucket' : 'Add Bucket'}
            </Button>
          </form>
        </Form>
      </DialogContent>
